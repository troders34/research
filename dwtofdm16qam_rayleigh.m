...
Fs = 200e3; % sample rate simulasi dalam Hz untuk pemenuhan syarat Doppler.
PathSamp         = [0 8 20]; % dalam satuan sample
PathDelays       = PathSamp/Fs; % mengonversi ke detik
AveragePathGains = [0 -3 -6]; % dalam satuan dB
% Hz diskrit (0 untuk kanal quasi-static) atau (<= Fs/10 utk lebih aman)
MaxDopplerShift  = 30;

% objek kanal Rayleigh fading
...
% meski opsional, reset kanal tiap run untuk reproduksibilitas
reset(raychan);

% mentransmisikan sinyal Tx ke kanal Rayleigh fading
rxFaded = raychan(txSig);

% menambahkan noise sesuai Eb/N0
rxSig = awgn(rxFaded, EbN0dB + 10*log10(k), 'measured');
...
for i=1:nCW
    [u8_dec_blk(:,i), uncorr] = dec(u8_rx_enc(:,i)); % dua outputs
    uncorr(i) = (uncorr < 0); % -1 = uncorrectable
end
u8_dec = u8_dec_blk(:);
